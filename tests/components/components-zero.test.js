...
// ── 0b0011  0  (F = 0, C = 0, N = 0, Z = 1) ─────────────────────────────
// All inputs are irrelevant — boundary-probed on all three to ensure no
// input can accidentally corrupt the constant output.
describe('ZERO', () => {
    for (const a of PROBE) {
        for (const b of PROBE) {
            for (const cin of CINS) {
                const result = runALU(0b0011, a, b, cin);
                test(`2I: a=0x${a.toString(16).padStart(2,'0')} b=0x${b.toString(16).padStart(2,'0')} cin=${cin}`, () => {
                    expect(result.i).toEqual({ f: 0, co: 0, no: 0, zo: 1 });
                });
                test(`2A: a=0x${a.toString(16).padStart(2,'0')} b=0x${b.toString(16).padStart(2,'0')} cin=${cin}`, () => {
                    expect(result.a).toEqual({ f: 0, co: 0, no: 0, zo: 1 });
                });
            }
        }
    }
